...
public class GameManager : MonoSingleton<GameManager>
{
    ...
    public int GetRandomTowerPositionIndex(int max = 20)
    {
        // 최대횟수 내에서 임의의 난수 생성
        var rand = -1;
        int count = 0;
    ...
    {
        Debug.Log("[func check] GameManager.AddTower");
    ...
    {
        // roads1 배열 내 임의의 요소 위치에 y 랜덤값 더한 후 반환
        var rand = GetRandomTowerPositionIndex(roads1.Count);
        //var position = roads1[rand].transform.localPosition + new Vector3(0, Util.Random(-1.5f, 1.5f));
        var flag = Util.Random(-1.0f, 1.0f);
        var position_x = roads1[rand].transform.localPosition.x + 1;
        var position_y = roads1[rand].transform.localPosition.y + (flag >= 0 ? 50.0f : -50.0f);
        var position = new Vector3(position_x, position_y); //new Vector3(0, Util.Random(-1.5f, 4.0f));
    ...
    {
        Debug.Log("[func check] GameManager.SetTower");
        //towers.Last().towerId = towerId;
        var tower = AddTower(randomTowerPosition.x, randomTowerPosition.y, ePlayer.me);

        int towerType = towerId / 10000;

        switch (towerType)
        {
            case 1:
                tower.Init(DataManager.instance.GetData<TowerDataSO>("TOW00001"));
                tower.SetRendererColor("white");
                break;
            case 2:
                tower.Init(DataManager.instance.GetData<TowerDataSO>("TOW00002"));
                tower.SetRendererColor("yellow");
                break;
            default:
                tower.Init(DataManager.instance.GetData<TowerDataSO>("TOW00001"));
                break;
        }
        tower.InitTowerType(towerType);
        ...
        // 첫 번째 자식 오브젝트에서 SpriteRenderer 찾기
        //var spriteRendererObj = tower.transform.Find("spriteRenderer");
        //if (spriteRendererObj != null)
        //{
        //    var spriteRenderer = spriteRendererObj.GetComponent<SpriteRenderer>();
        //    if (spriteRenderer != null)
        //    {
        //        spriteRenderer.color = Color.yellow; // 원하는 색상으로 변경
        //    }
        //}
    }
    ...
    IEnumerator MultiGameLoop()
    {
        ...
        for(int i = 0; i < 2; i++)
        {
            ...
            var roadLastPoint = roads.Last().position;
            gameObjects.endPosition.position = new Vector3(roadLastPoint.x - 10, roadLastPoint.y);
        ...
        while (isGameStart)
        {
            ...
            Debug.Log($"[loop check] initialGameState.MonsterSpawnInterval: {initialGameState.MonsterSpawnInterval}\n" +
                $"is packet null?: {packet == null}" +
    ...
    public void RemoveMonster(int monsterId)
    {
        ...
        if (targetMonster != null)
        {
            ...
            Debug.Log("[GM] GameManager.RemoveMonster :: targetMonster is not NULL");
        }
        else
            Debug.Log("[GM] GameManager.RemoveMonster :: targetMonster is NULL");
    }
